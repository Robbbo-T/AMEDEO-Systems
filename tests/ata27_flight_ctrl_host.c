...
{
    CtrlIn  xin;
    CtrlOut y_cpu, y_fpga, y_dsp, y_out;
} Ctx;

static CtrlOut law_cpu(const CtrlIn *x)
{
    CtrlOut y = {x->pitch_cmd * 0.8f, x->pitch_cmd * 0.8f};
    return y;
}
...
static int perceive(uint64_t t, void *ctx)
{
    Ctx *c = (Ctx *)ctx;
    (void)t;
    return hal_read_sensors(&c->xin, t);
}

static int observe(uint64_t t, void *ctx)
{
    (void)t;
    (void)ctx;
    return 0;
...
static int evolve(uint64_t t, void *ctx)
{
    (void)t;
    (void)ctx;
    return 0;
...
int main(void)
{
    ...
    const int period_us = 1000; /* 1 kHz */

    for (int i = 0; i < steps; i++)
    {
        uint64_t t = hal_now_us();

        if (poae_run_cycle(t, perceive, observe, actuate, evolve, &ctx) != 0)
        {
            return 1;
        }

        /* TSN metrics (synthetic) - scheduling requirement: jitter ≤ 50 us */
        uint32_t lat, jit;
        tsn_measure(&lat, &jit);
        if (jit > 50u)
        {
            fprintf(stderr, "[TSN] jitter too high: %u us > 50 us\n", jit);
            return 2;
        }

        /* busy-wait to approximate 1 kHz in host */
